  ...
  <div>
    <v-form>
      <v-row>
        ...
        >
          <v-checkbox
            v-model="isTokenPrizes"
            label="Create Token Prizes"
            :disabled="!isItemEditable"
            @change="update"
          />      
        </v-col>
    ...
    >
      <TokenPrizesBuilder
        :key="componentKey"
        :initial-item-data="[]"
        @tokenPrizesBuilder="handleTokenPrizes"
...
import TokenPrizesBuilder from './TokenPrizesBuilder'
...
export default {
  ...
  methods: {
    handleTokenPrizes(val) {
      this.initialItemData.tokenPrizes = val
      console.log(val)
    },
    resetValidation () {
